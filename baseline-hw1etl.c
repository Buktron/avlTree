...
int main (int argc, char **argv)
{
	...
	FILE *fp;
	int i = 0, j = 0, k = 0, count = 0, lineSkipper = 0, airLength = 0, heliPad = 0, header = 0;
	char buff[1024];  
	char *tmp, *tmp1;
    ...
    else
    {
        ...
        for(k = (heliPad + 1); k < airLength; k++)
        {
            ...
            if ((((air+k)->LocID[0] <= '9') && ((air+k)->LocID[0] >= '0')) || (((air+k)->LocID[1] <= '9') && ((air+k)->LocID[1] >= '0')) || (((air+k)->LocID[2] <= '9') && ((air+k)->LocID[2] >= '0')) || (((air+k)->latitude < 0.00) && ((air+k)->latitude > 31.00)))
            {
                heliPad++;
            }
            else
            {
                ...
                currentAirport->nextAirport->airPtr = (air+k);
    ...
    {
    sortByLatitude(airHead, airLength);
    }
    else if (sortArg == 'A' || sortArg == 'a')
    {
    sortByLocID(airHead, airLength);
    }
    else
    {
